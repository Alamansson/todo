...
func (r *todoListPostgres) GetAll(userId int) ([]todo.TodoList, error) {
	...
	query := fmt.Sprintf("SELECT tl.id, tl.title, tl.description FROM %s tl INNER JOIN %s UL "+
		"ON tl.id = ul.list_id WHERE ul.user_id=$1", todoListsTable, usersListTable)
...
func (r *todoListPostgres) GetById(userId, listId int) (todo.TodoList, error) {
	...
	query := fmt.Sprintf("SELECT tl.id, tl.title, tl.description FROM %s "+
		"tl INNER JOIN %s ul ON tl.id = ul.list_id WHERE ul.user_id=$1 AND ul.list_id=$2", todoListsTable, usersListTable)
...
func (r *todoListPostgres) Delete(userId, listId int) error {
	query := fmt.Sprintf("DELETE FROM %s tl USING %s ul WHERE tl.id=ul.list_id "+
		"AND ul.user_id=$1 AND ul.list_id=$2", todoListsTable, usersListTable)
...
func (r *todoListPostgres) Update(userId, listId int, input todo.UpdateListInput) error {
	...
	query := fmt.Sprintf("UPDATE %s tl SET %s FROM %s ul WHERE tl.id = ul.list_id AND ul.list_id=$%d AND ul.user_id=$%d", todoListsTable, setQuery, usersListTable, argId, argId+1)
